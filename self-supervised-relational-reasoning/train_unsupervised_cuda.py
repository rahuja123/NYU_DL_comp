...
parser.add_argument("--gpu", default="3", type=int, help="GPU id in case of multiple GPUs")
...
if(args.method=="relationnet"):
    ...
    model = Model(feature_extractor, aggregation=args.aggregation)
...
def main():
    ...
    for epoch in range(args.epoch_start, args.epochs):
        loss_train, accuracy_train = model.module.train(epoch, train_loader) #<-- Each model must have a "train" method
        with open(log_file, "a") as myfile:
                myfile.write(str(epoch)+","+str(loss_train)+","+str(accuracy_train)+"\n")
        if(epoch%9==0):
            checkpoint_path = "./checkpoint/"+str(args.method)+"/"+str(args.dataset)+"/"+header+"_epoch_"+ str(epoch+1)+"multigpu.tar"
